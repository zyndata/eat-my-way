...
<div class="relative overflow-hidden rounded-xl border border-(--color-border)">
  ...
  >
    ...
    >
      <!-- Wrapped rather than clipped (emw-recipe-name), for the reason the planner sheet wraps:
           on a phone a recipe name is longer than the row and the name is what the row is for. -->
      <span class="emw-recipe-name block font-medium {missing ? 'text-(--color-ink-muted) italic' : ''}">
        {name}
      </span>
